...
public class ConcurrentHashMapUnsafePutAllDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 5; i++) {
            executor.execute(() -> {
                ...
                System.out.println(Thread.currentThread().getName() + " 认为需要补充: " + needToAdd);

                for (int j = 0; j < needToAdd; j++) {
                    batch.put(900 + j, 900 + j);
                }

                map.putAll(batch); // 批量插入不是原子操作
            });
        }
        ...
        System.out.println("实际size: " + map.size()); // 结果可能远大于1000
    }
}
